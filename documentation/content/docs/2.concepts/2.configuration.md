...
Configure GraphQL API testing within the backend section. The endpoint URL is defined in the environment configuration:

```yaml
# In environments section
environments:
  local:
...
> 💡 **Endpoint Location:** The GraphQL endpoint URL must be defined in your environment configuration as `graphql_endpoint`, not in the backend section.
