...
Install browser drivers based on configuration.

```bash
tkit install [options]
```
...
The `install` command automatically installs the browser driver specified in your configuration file:

- **Rod Driver**: Reports as bundled (no installation needed)
- **Playwright Driver**: Downloads Chromium and installs system dependencies

#### Prerequisites

- **Playwright driver** requires Go 1.19+ and internet connection
- **Rod driver** has no prerequisites (bundled with TestFlowKit)

#### Examples

```bash
# Install driver from default config
tkit install

# Install driver from specific config
tkit install --config staging.yml

# Install Playwright driver (if configured)
...
::alert{type="info"}
Run this command once per driver. Re-run only when:
- Switching drivers (rod ↔ playwright)
- After TestFlowKit updates
- After Playwright version changes
::
