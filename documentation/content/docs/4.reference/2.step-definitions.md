...
When I store the response path "data.id" from the response into "variable_name" variable
...
Steps for REST API testing. Response field assertions use **GJSON** for JSON and **XPath** for XML.
...
Then the response should have field "data.field"
...
Then the response field "data.user.name" should be "John"
```
