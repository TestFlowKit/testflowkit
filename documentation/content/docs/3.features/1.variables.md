...
- ✅ **Random Data** — Generate dynamic values inline with `{{ rand:type:options }}` — see [Random Data Generation](/docs/features/random-data-generation)
...
Extract data from **JSON** or **XML** API responses using response paths:

```gherkin
# JSON response with GJSON syntax
Given I prepare a request for the "get_user" endpoint
When I send the request
And I store the response path "user.name" from the response into "user_name" variable
And I store the response path "user.email" from the response into "user_email" variable

# XML response with XPath syntax
And I store the response path "//user/id" from the response into "user_id" variable
```

#### Response Path Syntax

TestFlowKit uses [GJSON](https://github.com/tidwall/gjson) for JSON responses and XPath for XML responses.

| Expression | Description | Example |
|------------|-------------|---------|
| `name` | Root level field | `name` → `"John"` |
| `user.email` | Nested field | `user.email` |
| `items.0` | Array index | `items.0` → first item |
| `items.#` | Array length | `items.#` → `3` |
| `items.#.name` | All items field | `items.#.name` → all names |

> 💡 For advanced queries, filters, and modifiers, see the [GJSON Path Syntax documentation](https://github.com/tidwall/gjson/blob/master/SYNTAX.md)

::alert{type="info"}
Use `I store the response path ...` for new scenarios. The older `I store the JSON path ...` sentence remains supported for backward compatibility with JSON-based tests.
::
...
- [Random Data Generation](/docs/features/random-data-generation) — Generate UUIDs, emails, phone numbers, dates, and more inline
- [Macros](/docs/features/macros) — Create reusable test patterns
- [API Testing](/docs/features/api-testing) — REST API testing features
- [Step Definitions](/docs/reference/step-definitions) — Variable-related steps
