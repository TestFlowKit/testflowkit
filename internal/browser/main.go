...
func searchForSelector(ctx contextWrapper, mu *sync.RWMutex, p page, selector config.Selector, ch chan<- element) {
	...
	if selector.IsXPath() {
		elt, err = p.GetOneByXPath(selector.Value)
	} else {
		elt, err = p.GetOneBySelector(selector.Value)
	}

	if err != nil {
		logger.Warn(fmt.Sprintf("element not found with %s selector %s", selector.Type, selector.Value), []string{
