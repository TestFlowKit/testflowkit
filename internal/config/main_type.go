...
func (c *Config) GetGraphQLHeaders() map[string]string {
	...
	// Start with backend default headers
	for k, v := range c.Backend.DefaultHeaders {
		headers[k] = v
	}

	// Override with GraphQL-specific headers if GraphQL config exists
	if c.Backend.GraphQL != nil {
		for k, v := range c.Backend.GraphQL.DefaultHeaders {
			headers[k] = v
		}
	}
...
func (c *Config) validateGraphQL() error {
	...
	for operationName, operation := range c.Backend.GraphQL.Operations {
		...
		if operation.Type != "query" && operation.Type != "mutation" {
			return fmt.Errorf("GraphQL operation '%s' type must be 'query' or 'mutation', got '%s'", operationName, operation.Type)
		}

		if operation.Operation == "" {
			return fmt.Errorf("GraphQL operation '%s' must have an operation definition", operationName)
		}

		if operation.Description == "" {
			return fmt.Errorf("GraphQL operation '%s' must have a description", operationName)
		}
	}
