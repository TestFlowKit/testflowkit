...
/**
 * Calls the testflowkit CLI to export config schema as JSON.
 *
 ...
 */
export function fetchConfigSchemaFromCli(
  cliBinary: string,
  cwd: string,
): string {
  return execSync(`${cliBinary} export-config-schema --format json --silent`, {
    shell: "/bin/sh",
    cwd,
    encoding: "utf-8",
